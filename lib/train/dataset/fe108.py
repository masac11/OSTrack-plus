...
class FE108(BaseVideoDataset):
    ...
    def _get_sequence_list(self):
        with open(os.path.join('/data/yinhf/adjust/FE108/train/', 'list.txt')) as f:
    ...
    def get_sequence_info(self, seq_id):
        ...
        visible_ratio = torch.ones_like(bbox[:, 0])
        visible = torch.ones_like(bbox[:, 0])
        return {'bbox': bbox, 'valid': valid, 'visible': visible, 'visible_ratio': visible_ratio}

    def _get_frame_path(self, seq_path, frame_id):
        if seq_path.split('/')[-1] == 'stack':
            return os.path.join(seq_path, '{:05}.jpg'.format(frame_id+1))
    ...
    def _get_rgb_img_sequence_path(self, seq_id):
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        seq_rgb_path = self._get_rgb_img_sequence_path(seq_id)
        frame_list = [self._get_frame(seq_rgb_path, f_id) for f_id in frame_ids]
