...
class STARKProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        if self.transform['joint'] is not None:
            data['template_event'], data['template_anno'], data['template_masks'] = self.transform['joint'](
                image=data['template_event'], bbox=data['template_anno'], mask=data['template_masks'])
        ...
        for s in ['template', 'search']:
            ...
            # for suffix in ['_event']:
                assert self.mode == 'sequence' or len(data[s + suffix]) == 1, \
                ...
                crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + suffix], jittered_anno,
                                                                                data[s + '_anno'], self.search_area_factor[s],
                                                                                self.output_sz[s], masks=data[s + '_masks'])
                # Apply transforms
                if suffix != '_images':
                    data[s + suffix], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
                        image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
                else:
                    data[s + suffix], _, _, _ = self.transform[s](
                        image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
